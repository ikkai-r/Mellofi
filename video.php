 <?php
  session_start();
    ...
    if ($_GET['video_id'] == '') {
    header('Location: homepage.php?video_id=none');
  }
         ...
         $sql_subscription = "
                   ...
                   *
                   ...
                   ON 
                    `videos`.video_id = '".$_GET['video_id']."'
                   WHERE
                    `subscription`.musician_id = `videos`.musician_id
                   AND 
        ...
        <div class="modal-header" style="background-color: #101010">
          ...
          <?php
            ...
            $sql_artist = "
                ...
                `videos` 
                ON 
                `pins`.musician_id = `videos`.musician_id 
                INNER JOIN 
                `musician` 
                ON 
                `videos`.musician_id = `musician`.musician_id 
  ...
  <div class="modal fade" id="purchaseModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable" role="document">
      <div class="modal-content">
        ...
        <div class="mt-1">
          ...
          <video id="livestream" style="z-index: -1;" autoplay loop controls controlsList="nodownload noremoteplayback nopictureinpicture" disablePictureInPicture>
            <?php
                ...
                $result_videos = mysqli_query($conn, $sql_videos);
                    ...
                    for ($index = 0; $index < count($videos); $index++) {
                      echo '<source src="upload/videos/'.$videos[$index]["video_src"].'" type="video/mp4">';
                    }
        ...
        <div class="chatbox">
          <div class="stream-chat-hdr"> 
            <h4 class="header">COMMENT SECTION</h4>
          </div>
          ...
          <div style="margin-left: 60px;">
           ...
           <hr style="background-color: white">
            ...
            <div class="comment-view scroll-comments">
              <?php
              if (isset($_SESSION["isLogin"])) {
                if ($_SESSION["isLogin"] != "true") {
                   echo'<br><br><br><br>
              ...
              <form id="comments">
               ...
               <?php
                if (isset($_SESSION['isLogin'])) {
                   if ($_SESSION['isLogin'] == "true") {
                    echo '
                     <input class="form-control form-control-sm" id="commentmsg" name="commentmsg" type="text" placeholder="Say something!" style="margin-left: -10px;">
                    <button type="submit" id="commentBtn" name="commentBtn" style="margin-top: 9px; float: right; margin-right: 10px">Comment</button>
                    ...
                    echo '
                     <input class="form-control form-control-sm" id="commentmsg" name="commentmsg" type="text" placeholder="Say something!" data-toggle="modal" data-target="#logInModal" style="margin-left: -10px;">
                    <button type="button" data-toggle="modal" data-target="#logInModal" id="commentBtn" name="commentBtn" style="margin-top: 9px; float: right; margin-right: 10px">Comment</button>
                    <button type="button" id="wowBtn" name="wowBtn" style="margin-top: 9px; float: right; margin-right: 10px; "><i class="fas fa-surprise"></i></button>
                     <button type="button" id="hahaBtn" name="hahaBtn" style="margin-top: 9px; float: right; margin-right: 10px; "><i class="fas fa-grin-squint-tears"></i></button>
                     <button type="button" id="sadBtn" name="sadBtn" style="margin-top: 9px; float: right; margin-right: 10px; "><i class="fas fa-sad-cry"></i></button>
                      <button type="button" id="heartBtn" name="heartBtn" style="margin-top: 9px; float: right; margin-right: 10px; "><i class="fas fa-heart"></i></button>';
        ...
        <div class="stream-details-container row">
          <?php
               ...
               $video_user = [];
                ...
                $result_video_user = mysqli_query($conn, $sql_video_user);
                    while ($row = mysqli_fetch_assoc($result_video_user)) {
                      array_push($video_user, $row);
                    }

                    for ($index = 0; $index < count($video_user); $index++) {
                      echo '
            <div class="col-1">
            <a href="profile.php?musician_id='.$video_user[$index]["musician_id"].'"><img class="profile-img-live"src="upload/images/'.$video_user[$index]["profile_pic"].'" style="z-index: -1">
              </a>
          </div>
          <div class="col-11" style="margin-bottom: 20px">
              <p class="livestream-title">'.$video_user[$index]["title"].'</p>
              <a class="streamer" href="profile.php?musician_id='.$video_user[$index]["musician_id"].'" style="color: #EFC62E">'.$video_user[$index]["username"].'</a>
              <p class="type" style="text-transform: capitalize">'.$video_user[$index]["type"].'</p> ';                  
                    }
        ...
        <div class="social-media row">
          ...
          <div class="col-11" style="padding-top: 29px; padding-left: 140px">
             <?php
              ...
              $sql_socials = "
                 ...
                 INNER JOIN 
                  videos 
                 ON 
                  `social_media`.musician_id = `videos`.musician_id
                  WHERE
                   `videos`.video_id = '".$_GET['video_id']."'
              ";
    ...
    <script>
      ...
      function runFunction(){
        $.post("fetchcomment.php", { video_id: "<?php echo $_GET['video_id']; ?>" },
...
$(document).ready(function(){
      ...
      function runDisplay(){
        ...
        $.post("followDisplay.php", { video_id: video_id},
